...
export function CreateAccountPrompt({ orderId, defaultEmail }: { orderId: string; defaultEmail: string }) {
  ...
  if (status === "needs-confirmation") {
    return (
      <div className="mt-8 border border-primary/40 bg-primary-tint px-5 py-4 text-sm text-text">
        Almost there. Check your email to confirm your new account, then sign in to see this order
        under Order History and use Buy Again — it&apos;s safely on record either way.
      </div>
    );
  }
  ...
  return (
    <div className="mt-8 border border-border p-6">
      ...
      <p className="mt-1 text-sm text-text">
        Optional: create an account and this order will be attached to it, so you can track it and
        use Buy Again from your Order History next time.
      </p>
