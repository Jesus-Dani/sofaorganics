...
export interface ProductQueryRow {
  ...
  product_facets: { facets: { facet_type: FacetType; label: string; slug: string } | null }[];
}
...
export function mapProductRow(row: ProductQueryRow): Product {
  ...
  const images: ProductImage[] = [...row.product_images]
    ...
    .map((img) => ({ src: img.storage_path, alt: img.alt_text, isPlaceholder: false }));

  const facets: ProductFacetTag[] = row.product_facets
    .map((pf) => pf.facets)
    .filter((f): f is NonNullable<typeof f> => f !== null)
    .map((f) => ({ facetType: f.facet_type, label: f.label, slug: f.slug }));
...
export const PRODUCT_SELECT = `
  ...
  product_facets ( facets ( facet_type, label, slug ) )
`;
